export type IDockValue = 'mini' | 'bottom' | 'all';
export interface Show2HtmlOptions {
  /** Debug显示区css样式名 */
  cls?: string;
  /** 当前显示方式 */
  dock?: IDockValue;
...
export default function show2Html(Debug, options: Show2HtmlOptions = {}) {
  ...
  if (curdockSideIndex < 0) curdockSideIndex = 0;
  ...
  // .${className} .t-${className} .menu{display:none;position:absolute;right:0;top:22px;list-style-type:none;font-size:12px;line-height:1rem;background:rgba(0,0,0,.8);margin:0;padding:0}
  // .${className} .t-${className} .logo:hover .menu{display:block}
  // .${className}.mini .t-${className} .logo:hover .menu{top:auto;bottom:22px}
  // .${className} .t-${className} .menu li{padding:3px 5px;cursor:pointer}
  // .${className} .t-${className} .menu li:hover{background:#333;}
  // <ul class="menu"><li>mini</li><li>bottom</li><li>all</li></ul>
